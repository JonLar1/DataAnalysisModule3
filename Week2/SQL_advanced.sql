...
WITH product_revenue AS (
    ...
    WHERE o.status = 'paid'
    GROUP BY s.store_id, s.name, p.product_id, p.name, cat.name
),
ranked AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY store_id ORDER BY product_revenue DESC) AS rn
...
WITH order_seq AS (
    SELECT
        ...
        LAG(o.order_datetime) OVER (
            PARTITION BY o.customer_id ORDER BY o.order_datetime
...
FROM inventory i
JOIN units_sold us ON i.store_id = us.store_id AND i.product_id = us.product_id
